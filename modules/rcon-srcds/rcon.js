    ...
    RCON.prototype.write = function (type, id, body) {
        ...
        return new Promise(function (resolve, reject) {
            ...
            var onData = function (packet) {
                decodedPacket = packets.decode(packet, _this.encoding);
                const fs = require('fs');
                ...
                if (type === protocol_1.default.SERVERDATA_AUTH && decodedPacket.type !== protocol_1.default.SERVERDATA_AUTH_RESPONSE) {
                    // TODO: It seems, the CS2 server does not send this additional packet.
                    console.log('Does this still get called anyway?');
                    return;
                    ...
                    // if decoded packet has the correct id, and a termination string at the end,
                    //  it is a single packet answer
                    if (packet.indexOf("0000", -2 , "hex") != -1) {
                        response = decodedPacket.body.replace(/\n\s*$/, ''); // delete trailing empty line
                        _this.connection.removeListener('data', onData);
                        resolve(response);
                    }
                } else {
                    // If the sent packet is not of type SERVERDATA_AUTH
                    // AND
                    // The decoded packet ID does not match the sent id, it is the continuation of a multipacket response,
                    // so we keep reading until a termination string is detected.
                    response = response.concat(packet.toString(_this.encoding, 0, packet.byteLength - 2));
                    if (packet.indexOf("0000", -2 , "hex") != -1) {
                        _this.connection.removeListener('data', onData);
