...
class ServerInfo {
  ...
  mapDetails() {
    if (this.#mapFilters.length > 0) {
      return this.#mapsDetails.filter((map) => {
        let found = false;
        if (map.name) {
          // sometimes map.name is undefined for some reason.
          this.#mapFilters.forEach((filter) => {
  ...
  mapFilterRemove(itemToRemove) {
    ...
    if (
      typeof itemToRemove === "number" &&
      this.#mapFilters.length > parseInt(itemToRemove)
    ) {
      console.log("removing number");
      this.#mapFilters.splice(parseInt(itemToRemove), 1);
      this.serverInfoChanged.emit("change");
    } else {
      const newFilters = this.#mapFilters.filter((currentItem) => {
  ...
  addPlayer(newPlayer) {
    if (
      this.#players.find((x) => x.steamID === newPlayer.steamID) !== undefined
    ) {
      this.#players.find(
        (x) => x.steamID === newPlayer.steamID
      ).disconnected = false;
    } else {
      newPlayer.team = "U";
      newPlayer.kills = 0;
  ...
  assignPlayer(name, steamID, team) {
    if (this.#players.find((x) => x.steamID === steamID) === undefined) {
      this.addPlayer({ name, steamID });
    }
    const player = this.#players.find((x) => x.steamID === steamID);
  ...
  recordKill(killer, victim) {
    const killPlayer = this.#players.find((x) => x.steamID === killer);
    if (killPlayer !== undefined) {
      killPlayer.kills += 1;
    }
    const victimPlayer = this.#players.find((x) => x.steamID === victim);
